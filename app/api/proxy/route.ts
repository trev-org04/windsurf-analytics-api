...
import { ApiResponseData, RequestBody } from "./types";
    ...
    if (runs) {
      runs.forEach(({ day, promptsUsed }) => {
        ...
        if (!creditSpendPerDay[day]) {
          creditSpendPerDay[day] = Number(promptsUsed) / 100;
        } else {
          creditSpendPerDay[day] += Number(promptsUsed) / 100;
        }
      });
...
> = {
  ...
  option6: creditSpendQuery,
  option7: creditSpendQuery,
};
